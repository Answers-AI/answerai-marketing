...
import ContentPreview from '@answersai-marketing/components/src/components/ContentPreview/ContentPreview';
...
const fetchPreview = async (id: string, locale: string, environment: string) => {
  const previewGqlClient = new GraphQLClient(
    `${process.env.NODE_ENV === 'development' ? 'http://localhost:8888/graphql' : '/api/graphql'}?env=master`
  );
